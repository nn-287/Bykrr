...
class OrderController extends Controller
{
    public function list($status)
    {
        ...
        return view('merchant-views.order.list', compact('orders', 'status'));
    }
    ...
    public function details($id)
    {
        ...
        if (isset($order)) {
            return view('merchant-views.order.order-view', compact('order'));
        } else {
    ...
    public function search(Request $request)
    {
        ...
        return response()->json([
            'view' => view('merchant-views.order.partials._table', compact('orders'))->render()
        ]);
    }
    ...
    public function status(Request $request)
    {
        ...
        if (!$order) //handling when order is not found
        {
        ...
        {
            $delivery_charge_setting = BusinessSetting::where(['key' => 'delivery_charge'])->first();
            $points_value_setting = BusinessSetting::where(['key' => 'points_value'])->first();

            if (!$delivery_charge_setting || !$points_value_setting) 
            {
                Toastr::error('Delivery charge or points value setting not found!');
                return back();
            }

            $points_value = ($order->order_amount - $delivery_charge_setting->value) * $points_value_setting->value;
        ...
        if ($order->customer) //checks if it's available or null
        {
        ...
        {
            Toastr::error('Customer not found for the order!');
        }
    ...
    //     return view('merchant-views.order.invoice', compact('order', 'gift_discount'));
    // }
    ...
    public function view_invoice($branch_id, $order_id)
    {
        ...
        return view('merchant-views.order.invoice', compact('invoice'));
    }
    ...
    public function update_order_details(Request $request, $id)
    {
       ...
       if($gift!=null){
        ...
        $orderDetail->merchant_id = Auth::guard('merchant')->id(); // Authenticated merchant ID
        $orderDetail->save();
        $orderDetail->save();
    ...
    public function products_list(Request $request, $orderId)
    {
        ...
        return view('merchant-views.order.products-list', compact('products', 'search', 'orderId'));
    }
    ...
    public function add_product(Request $request, $orderId, $productId)
    {
        ...
        $merchantId = $order->merchant_id; // Get the merchant_id from the order
        ...
        $or_d = [
            ...
            'attributes' => $request->has('attribute_id') ? json_encode($request->attribute_id) : json_encode([]), // Set attributes
            'merchant_id' => $merchantId, // // Set merchant_id from the order
       ...
       if($gift['discount'] > 0){
        ...
        // return view('merchant-views.order-view', compact('products', 'search', 'orderId'));
        
        return redirect()->route('merchant.orders.details', [$orderId]);
    }
